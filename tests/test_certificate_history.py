...
class CertificateHistoryTests(unittest.TestCase):
    ...
    def test_generates_performance_page_shell(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertIn('id="speed-heading">Target boat speed</h3>', page)
            self.assertIn('id="vmg-heading">Best VMG targets</h3>', page)
            ...
            self.assertLess(page.index('id="vmg-heading"'), page.index('id="speed-heading"'))
            self.assertEqual(3, page.count("<table"))
            self.assertIn('id="speed-head"', page)
            self.assertIn('id="speed-body"', page)
            self.assertIn('<caption>Beat</caption>', page)
            self.assertIn('<caption>Run</caption>', page)
            self.assertIn('id="beat-targets"', page)
            self.assertIn('id="run-targets"', page)
            self.assertNotIn('id="vmg-head"', page)
            self.assertNotIn('id="vmg-body"', page)
            self.assertNotIn('id="target-matrix"', page)
            self.assertNotIn("Dual-angle speed polar", page)
            self.assertNotIn('class="guide-links', page)
